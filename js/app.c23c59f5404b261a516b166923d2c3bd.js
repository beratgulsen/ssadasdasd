	...
	$(document).ready(function() {
		...
		$('.beta-checkbox').on('change', function () {
			...
			apiPost(url, data, function (err) {
				if (err) return showError('An error occurred.');
				var enabledOrDisabled = enabled ? 'Enabled' : 'Disabled';
				var redirect = enabled ? 'https://beta.EternalBOTbot.net' : 'https://www.EternalBOTbot.net';

				showSuccess(enabledOrDisabled + ' EternalBOT Beta, redirecting you to ' + redirect);
